...
class Perfil : Fragment() {
    ...
    private fun recargar(view: View){
        viewModel.getUserMe().observeForever{
            if(it.body != null) {
                ...
                val adapterValoracion = ValoracionAdapter(user.valoraciones, resources,
                    onUserClick = { data ->
                        ...
                        findNavController().navigate(R.id.action_perfil2_to_perfiles)
                    })
                recyclerViewVloraciones.adapter = adapterValoracion
                view.findViewById<ImageButton>(R.id.btnUbicaicones).setOnClickListener{
                    findNavController().navigate(R.id.action_perfil2_to_ubicaciones2)
                }
            }
        }
    }
    ...
    class ValoracionAdapter(private val dataList: List<Valoraciones>, private var resources: Resources,
        ...
        override fun onBindViewHolder(holder: MyViewHolder2, position: Int) {
            ...
            holder.puntuacion.text = String.format("%.2f", data.score)
